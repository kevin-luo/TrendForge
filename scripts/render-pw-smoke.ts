...
// Import puppeteer-core from its pnpm store location
const puppeteer = await import("E:/coding/TrendForge/node_modules/.pnpm/puppeteer-core@25.1.0/node_modules/puppeteer-core/lib/esm/puppeteer/puppeteer-core.js");
...
const fps = 30;
const totalFrames = Math.round(specs.reduce((s:any,sp:any)=>s+sp.duration,0)*fps);
console.log(`Total: ${totalFrames}f`);

const EXEC = "C:/Program Files/Google/Chrome/Application/chrome.exe";
const browser = await puppeteer.default.launch({ executablePath: EXEC, headless: true, args: ["--no-sandbox","--disable-gpu","--disable-setuid-sandbox"] });
const page = await browser.newPage();
await page.setViewport({ width: 1080, height: 1920, deviceScaleFactor: 2 });
await page.goto("file:///" + htmlPath.replace(/\\/g,"/"), { waitUntil: "networkidle0" });

const keys = [0, Math.round(totalFrames*0.15), Math.round(totalFrames*0.35), Math.round(totalFrames*0.55), Math.round(totalFrames*0.75), Math.max(0,totalFrames-1)];
for (const f of keys) {
